    ...
    if i == 1:
      ...
      if start > last:
        ...
        rand_attn[i - 1, :] = np.random.permutation(
            np.concatenate((a[:start], a[end + 1 : last]))
        )[:r]
  return rand_attn
...
def band_start_block_rand_multi_attention_pad(
    query_matrix,
    key_matrix,
    value_matrix,
    rand_attn,
  ...
  gathered_key = jnp.reshape(
      gather_2(blocked_key_matrix, rand_attn), (b, h, m // wm - 2, r * wn, -1)
  )  # [b, h, n//wn-2, r, wn, -1]
  # tf.gather(
  #   blocked_value_matrix, rand_attn, batch_dims=2, name='gather_value')
  gathered_value = jnp.reshape(
      gather_2(blocked_value_matrix, rand_attn), (b, h, m // wm - 2, r * wn, -1)
  )  # [b, h, n//wn-2, r, wn, -1]

  first_product = jnp.einsum(
      'BHQD,BHKD->BHQK', blocked_query_matrix[:, :, 0], key_matrix
  ...
  first_context_layer = jnp.einsum(
      'BHQK,BHKD->BHQD', first_attn_weights, value_matrix
  ...
  first_band_product += (
      1.0 - jnp.expand_dims(seq_n_pad[:, :, :, :wn], 3)
  ) * -10000.0
  last_band_product += (
      1.0 - jnp.expand_dims(seq_n_pad[:, :, :, -wn:], 3)
  ) * -10000.0
  ...
  last_product = jnp.einsum(
      'BHQD,BHKD->BHQK', blocked_query_matrix[:, :, -1], key_matrix
  )  # [b, h, wm, -1] x [b, h, n, -1] ==> [b, h, wm, n]
  ...
  last_context_layer = jnp.einsum(
      'BHQK,BHKD->BHQD', last_attn_weights, value_matrix
  )  # [b, h, wm, n] x [b, h, n, -1] ==> [b, h, wm, -1]
...
def sparse_dot_product_attention(
  ...
  queries = jnp.pad(
      queries, ((0, 0), (0, seq_length - from_seq_length), (0, 0), (0, 0))
  )
  keys = jnp.pad(
      keys, ((0, 0), (0, seq_length - to_seq_length), (0, 0), (0, 0))
  )
  values = jnp.pad(
      values, ((0, 0), (0, seq_length - to_seq_length), (0, 0), (0, 0))
  )
  ...
  if input_mask is None:
    ...
    input_mask = jnp.pad(
        input_mask,
        tuple(
            (0, seq_length - size) if i == 1 else (0, 0)
  ...
  # create band padding
  band_pad = create_band_mask_from_inputs(
...
class BigBirdAttention(nn.Module):
  ...
  def __call__(
    ...
    if self.rope_pos_emb:
      ...
      sin, cos = generate_fixed_pos_embedding(dim, max_length)
...
class BigBirdSelfAttention(BigBirdAttention):
  ...
  @nn.compact
  def __call__(
      self, inputs_q, padding_mask=None, segmentation=None, dropout_rng=None
  ):
    """Applies multi-head dot product attention on the input data.
