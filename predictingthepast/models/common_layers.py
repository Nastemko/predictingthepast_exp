...
class Embed(nn.Module):
  ...
  def __call__(self, inputs, num_embeddings, features):
    ...
    embedding = self.param(
        'embedding', self.emb_init, (num_embeddings, features)
    )
    if self.mode == 'input':
...
class AddPositionEmbs(nn.Module):
  ...
  def __call__(self, inputs, inputs_positions=None, cache=None):
    ...
    if self.posemb_dim is None or self.combine_type == 'add':
      posemb_dim = inputs.shape[-1]
    else:
      posemb_dim = self.posemb_dim
    pos_emb_shape = (1, self.max_len, posemb_dim)
...
class MlpBlock(nn.Module):
  ...
  def __call__(self, inputs):
    ...
    if self.activation_fn == 'geglu':
      ...
      x = nn.gelu(x[..., : self.mlp_dim]) * x[..., self.mlp_dim :]
