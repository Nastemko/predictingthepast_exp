...
def date_to_bin(date_cur, date_min, date_max, date_interval, date_bins):
  if date_cur >= date_min and date_cur < date_max:
    date_bin = np.digitize(
        date_cur, list(range(date_min + date_interval, date_max, date_interval))
    )
  else:
...
def date_range_to_dist(
  ...
  if (
      date_min_cur
      and date_max_cur
      and date_min_cur >= date_min
      and date_max_cur < date_max
      and date_min_cur <= date_max_cur
  ):
    date_min_cur_bin = date_to_bin(
        date_min_cur, date_min, date_max, date_interval, date_bins
    )
    date_max_cur_bin = date_to_bin(
        date_max_cur, date_min, date_max, date_interval, date_bins
    )
  else:
  ...
  dist[date_min_cur_bin : date_max_cur_bin + 1] = 1.0 / date_bins_cur

  if return_logits:
    eps = 1e-6
    dist = np.clip(dist, eps, 1.0 - eps)
    dist = np.log(dist)
