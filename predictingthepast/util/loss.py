...
def categorical_kl_divergence(p_logits, q_logits, temperature=1.0):
...
def cross_entropy_label_smoothing_loss(
    logits, labels, mask=None, label_smoothing=0.0
):
...
def cross_entropy_mask_loss(logits, label, mask):
  nll = -jax.nn.log_softmax(logits)[label]
  loss = jnp.multiply(nll, mask.astype(logits.dtype))
  return loss


def date_loss_l2(pred, target_min, target_max, mask):
...
def date_loss_l1(pred, target_min, target_max, mask):
...
def poisson_loss(y_pred, y_true, mask):
  """Poisson loss`."""
  loss = y_pred - y_true * jnp.log(y_pred + 1e-7)

  # Mask loss
  assert mask.ndim == loss.ndim
  loss = jnp.multiply(loss, mask.astype(loss.dtype))
  return loss
